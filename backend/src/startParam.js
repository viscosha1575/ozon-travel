...
export function buildStartParam({ referralCode = "", utmSlug = "" } = {}) {
  const normalizedReferralCode = normalizeReferralCode(referralCode);
  const normalizedUtmSlug = normalizeUtmSlug(utmSlug);
  const parts = [];

  if (normalizedReferralCode) {
    parts.push(`ref_${normalizedReferralCode}`);
  }

  if (normalizedUtmSlug) {
    parts.push(`utm_${normalizedUtmSlug}`);
  }

  return parts.join("__");
}

export { normalizeReferralCode, normalizeUtmSlug };
